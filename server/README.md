...
Режим «1 на 1 с другом». Сервер авторитетный: бой считает он тем же кодом, что и игра (`src/game/match.js`), телефоны шлют только команды (джойстик, удар, ульта). Протокол — `src/net/protocol.js`, устройство — `docs/ARCHITECTURE.md`.
