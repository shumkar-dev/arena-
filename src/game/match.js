...
// Сейчас контроллеры — кнопки (local) и бот (bot). Для сетевой игры появится
// третий — remote: команды приходят по сети, а симуляция остаётся той же.
...
export function createMatch({ scene, fx, mode, options = {} }) {
  ...
  const match = {
     ...
     * control: 'local' — кнопки этого устройства, 'bot' — бот, позже 'remote' — игрок по сети.
     */
